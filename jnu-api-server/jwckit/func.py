...
def login(username, password):
        ...
        headers = {'User-agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)',
                   "Referer": "http://202.116.0.176/login.aspx"}
        #创建一个浏览器会话
        s = requests.session()

        #发起GET请求
        html = s.get("http://202.116.0.176/login.aspx").text
        ...
        for i in range(10):
            #print i
            con = s.get("http://202.116.0.176/ValidateCode.aspx").content
...
def get_person_teacher(s,xuehao):
    html = s.get("/Secure/Xjgl/Xjgl_Zsgl_Xxcx.aspx").text
    soup = BeautifulSoup(html)
    dic = {}
    #搜索表单并填写
    ...
    for i in ['txtND','ddListZSJJ','txtXZ','txtKSH','ddListPYCC','ddListSYLB','ddlGATQ','txtXM','ddlXQ','ddlXY','ddlDRFS']:
        dic[i]=""
    html = s.post(
        "http://202.116.0.176//Secure/Xjgl/Xjgl_Zsgl_Xxcx.aspx",
        data=dic
    ).text
...
def get_socre_teacher(s):
    html = s.get("http://202.116.0.176//Secure/Cjgl/Cjgl_Cjcx_Xscjlbcx.aspx").text
    soup = BeautifulSoup(html)
    dic = {}
    #搜索表单并填写
    for i in soup.findAll("input"):
    ...
    html = s.post(
        "http://202.116.0.176//Secure/Cjgl/Cjgl_Cjcx_Xscjlbcx.aspx",
        data=dic
    ).text
...
def get_xls(s, age, qx):
    ...
    html = s.get("http://202.116.0.176//Secure/PaiKeXuanKe/wfrm_XK_MainCX.aspx").text
    soup = BeautifulSoup(html)
    dic = {}
    ...
    #发起POST请求
    html = s.post(
        "http://202.116.0.176//Secure/PaiKeXuanKe/wfrm_XK_MainCX.aspx",
        data=dic
    ).text
...
def get_kaoshi(s, age, qx):
    ...
    html = s.get("http://202.116.0.176//Secure/PaiKeXuanKe/wfrm_xk_StudentKcb.aspx").text
    soup = BeautifulSoup(html)
    dict1 = {}
    ...
    #发起POST请求
    html = s.post(
        "http://202.116.0.176//Secure/PaiKeXuanKe/wfrm_xk_StudentKcb.aspx",
        data=dict1
    ).text
    #原页面为多级iframe嵌套，所以这里可能难于理解
...
def get_info(s):
    html = s.get('http://202.116.0.176//Secure/Xjgl/Xjgl_Xsxxgl_Xjxxxg_XS.aspx').text
    soup = BeautifulSoup(html)
    str = ""
    data={}
    for i in soup.findAll("input"):
